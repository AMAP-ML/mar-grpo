...
def to_paddle_device(torch_dev: torch.device) -> str:
    # torch_dev 可能是 device(type='cuda', index=1) 或 device(type='cpu')
    if torch_dev.type == "cuda":
        idx = torch_dev.index
        if idx is None:  # 有时为 None，用当前设备兜底
            idx = torch.cuda.current_device()
        return f"gpu:{idx}"
    elif torch_dev.type == "cpu":
        return "cpu"
    else:
        # 其他类型（如 mps/xpu），按需映射；这里先退回 CPU
        return "cpu"
...
class OcrScorer:
    ...
    def load_to_device(self, load_device=None):
        # device_str=to_paddle_device(load_device)
        # paddle.set_device(device_str)  # 指定第1号GPU
        self.ocr = PaddleOCR(
            use_angle_cls=False,
...
if __name__ == "__main__":
    images=[]
    image_root=os.environ.get("OCR_DEMO_IMAGE_ROOT", "./ocr_demo/raw")
