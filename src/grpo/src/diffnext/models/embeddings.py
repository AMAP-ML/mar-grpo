...
class MaskEmbed(nn.Module):
    ...
    def get_pred_mask(self, num_preds) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        pred_mask = torch.zeros_like(self.mask).scatter_(1, pred_ids, 1)#对每行，在 pred_ids 指定的位置写 1，其他为 0，得到本轮要预测的位置掩码。
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        if self.mask is None:#初始时self.mask为0，用全1矩阵初始化，初始的pred_pos为0
            self.mask, self.pred_pos = x.new_ones(x.shape[:-1] + (1,)), 0
        return self.apply_mask(x)

    def apply_with_prev_ids(self, x: torch.Tensor, prev_ids: torch.Tensor):
        """给定 x 和 prev_ids，重置 mask=1 并在 prev_ids 处置 0，返回 masked x 与 prev_ids。"""
        if prev_ids.dim() == 2:  # [B, K] -> [B, K, 1]
            prev_ids = prev_ids.unsqueeze(-1)

        # 重置为全 1（可预测），在 prev_ids 位置置 0（保留原值）
        self.mask = x.new_ones(x.shape[:-1] + (1,))
        self.mask.scatter_(1, prev_ids, 0)
