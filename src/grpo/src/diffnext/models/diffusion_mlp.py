...
class DiffusionMLP(nn.Module):
    ...
    def forward(self, x, timestep, z, pred_ids=None) -> torch.Tensor:
        x, o = self.patch_embed(x), None if pred_ids is None else x#如果pred_ids是None，o就是None，否则完整保留一份x
        o = None if pred_ids is None else self.patch_embed.patchify(o)
        x = x if pred_ids is None else x.gather(1, pred_ids.expand(-1, -1, x.size(-1)))#只取pred_ids对应位置的token
        ...
        # 如果pred_ids是None就输出整个处理后的x（此时x没有筛选），否则返回仅pred_ids对应区域被denoise的x