...
def sde_step_with_logprob(
    ...
    sigma = self.sigmas[step_index].view(-1, 1, 1, 1).to(sample.device)
    sigma_prev = self.sigmas[prev_step_index].view(-1, 1, 1, 1).to(sample.device)
    ...
    # 由于mar还需要考虑token的问题，我们先不在logp的层面上算均值
    # log_prob = log_prob.mean(dim=tuple(range(1, log_prob.ndim)))

    prev_sample=prev_sample.to(sample.dtype)
    log_prob=log_prob.to(sample.dtype)
    prev_sample_mean=prev_sample_mean.to(sample.dtype)
    std_dev_t_dt=(std_dev_t * torch.sqrt(-1*dt)).to(sample.dtype)
